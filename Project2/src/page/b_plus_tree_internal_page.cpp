...
void B_PLUS_TREE_INTERNAL_PAGE_TYPE::Init(page_id_t pid,page_id_t paid)
{
  SetPageType(IndexPageType::INTERNAL_PAGE);
  SetSize(0);
  SetPageId(pid);
  SetParentPageId(paid);
 ...
 */
INDEX_TEMPLATE_ARGUMENTS
KeyType B_PLUS_TREE_INTERNAL_PAGE_TYPE::KeyAt(int ind) const
{
...
INDEX_TEMPLATE_ARGUMENTS
void B_PLUS_TREE_INTERNAL_PAGE_TYPE::SetKeyAt(int ind, const KeyType &k)
{
  assert(ind >= 0 && ind < GetSize());
  array[ind].first = k;
}
 ...
 */
INDEX_TEMPLATE_ARGUMENTS
int B_PLUS_TREE_INTERNAL_PAGE_TYPE::ValueIndex(const ValueType &val) const
{
  for(int i = 0; i < GetSize(); i++)
  {
    if (val != ValueAt(i)) continue;
    return i;
  }
  return -1;
}
 ...
 */
INDEX_TEMPLATE_ARGUMENTS
ValueType B_PLUS_TREE_INTERNAL_PAGE_TYPE::ValueAt(int ind) const
{
  assert(ind >= 0 && ind < GetSize());
...
INDEX_TEMPLATE_ARGUMENTS
ValueType
B_PLUS_TREE_INTERNAL_PAGE_TYPE::Lookup(const KeyType &k,const KeyComparator &comparator) const
{
  assert(GetSize() > 1);
  ...
  while (start <= end)
  {
    ...
    if (comparator(array[mid].first,k) <= 0) 
    {
      start = mid + 1;
    }
